...
function extractGoogleApiErrorDetails(error) {
  ...
  const nestedReasons = details.map(detail => detail?.reason || detail?.metadata?.reason).filter(Boolean).map(reason => String(reason).toLowerCase());
  const legacyErrors = Array.isArray(resultError?.errors) ? resultError.errors : [];
  const legacyReasons = legacyErrors.map(item => item?.reason).filter(Boolean).map(reason => String(reason).toLowerCase());
  const reasons = [...legacyReasons, ...nestedReasons];
  const message = resultError?.message || error?.message || error?.statusText || (typeof error === "string" ? error : "");
...
function maybeWrapDriveInitError(error) {
  ...
  const reasons = Array.isArray(details.reasons) ? details.reasons.map(reason => String(reason).toLowerCase()) : [];
  const currentLocation = typeof window !== "undefined" && window.location ? `${window.location.origin}${window.location.pathname}` : "this site";
  ...
  if (lowerMessage.includes("api key not valid") || lowerMessage.includes("invalid key")) {
    return buildWrappedError("Google reported that the Drive API key is invalid. Make sure you pasted the full key, the Drive API is enabled for that Cloud project, and try saving it again.", "drive_discovery_invalid_key");
  }
  return null;
}
...
function stableStringify(value) {
  ...
  const walk = input => {
    ...
    const entries = Object.keys(input).sort().map(key => `${JSON.stringify(key)}:${walk(input[key])}`);
    return `{${entries.join(",")}}`;
  };
  try {
    return walk(value);
...
function useDriveSync({
  ...
  const setDriveState = useCallback(updater => {
  ...
  const uploadSnapshot = useCallback(async payload => {
    const gapi = gapiRef.current;
    if (!gapi) throw new Error("Google API not ready");
    const fileId = await ensureFileId();
    try {
  ...
  const requestAccessToken = useCallback(({
    ...
    tokenClient.callback = tokenResponse => {
      ...
      if (gapiRef.current) {
        gapiRef.current.client.setToken({
  ...
  const syncNow = useCallback(async ({
    ...
    if (driveState.status === "disabled") {
      if (!silent) {
        const message = driveState.disabledReason === "drive_api_disabled" ? DRIVE_DISABLED_MESSAGE : "Google Drive sync is not available.";
        notify?.({
          type: "info",
          message,
          autoCloseMs: 8000
    ...
    if (!driveState.signedIn) {
      if (!silent) notify?.({
        type: "warning",
        message: "Connect Google Drive to sync data."
      });
      return {
        ok: false
      };
  ...
  const signIn = useCallback(async () => {
    if (driveState.status === "disabled") {
      if (driveState.disabledReason === "drive_api_disabled") {
        ...
        notify?.({
          type: "warning",
          message: "Google Drive sync is not available."
        });
      }
      return false;
    }
    if (!tokenClientRef.current) {
      notify?.({
        type: "warning",
        message: "Google Drive isn’t ready yet."
      });
      return false;
    }
    try {
      ...
      if (!error?.code || !["access_denied", "popup_closed_by_user", "user_cancelled"].includes(String(error.code))) {
        notify?.({
          type: "error",
          message: `Google sign-in failed: ${error?.message || error}`
        });
      }
      return false;
    }
  }, [driveState.disabledReason, driveState.status, handleSignedIn, handleSignedOut, isTokenAuthError, notify, requestAccessToken]);
  const signOut = useCallback(async () => {
    ...
    try {
      ...
      notify?.({
        type: "error",
        message: `Sign-out failed: ${error?.message || error}`
      });
      return false;
    }
...
function App() {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "ml-auto flex flex-col gap-2 text-sm text-zinc-500 dark:text-zinc-400 sm:flex-row sm:items-center sm:gap-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap items-center justify-end gap-2"
  }, driveSync.available ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    className: "btn",
    type: "button",
    onClick: driveSync.signedIn ? driveSync.signOut : driveSync.signIn,
    disabled: driveSync.status === "loading" || driveSync.syncing
  }, driveSync.status === "loading" ? "Connecting…" : driveSync.signedIn ? "Disconnect Drive" : "Connect Drive"), /*#__PURE__*/React.createElement("button", {
    className: "btn",
    type: "button",
    ...
    title: "Toggle theme"
  }, theme === "dark" ? "☀️ Light" : "🌙 Dark")), driveSync.signedIn && /*#__PURE__*/React.createElement("div", {
    className: "flex justify-end text-xs text-emerald-700/80 dark:text-emerald-300/80"
  }, driveSync.syncing ? "Syncing with Google Drive…" : driveSync.lastSync ? `Synced ${formatRelativeTimeLabel(driveSync.lastSync)}` : "Not synced yet"), driveSync.status === "error" && /*#__PURE__*/React.createElement("div", {
    className: "flex justify-end text-xs text-rose-600 dark:text-rose-300"
  }, "Drive connection error \u2014 try reconnecting."))))), /*#__PURE__*/React.createElement("main", {
